...
# ---- Uninstaller name patterns to exclude (case-insensitive) ----
...
# Skips uninstallers, logs conflicts but does not overwrite
function Copy-ToApplications($file) {
    if (Is-Uninstaller $file) {
        Write-Host "   [SKIPPED - Uninstaller] $($file.Name)" -ForegroundColor DarkGray
        return
    }
...
# Skips uninstallers. Logs conflicts without overwriting.
# ------------------------------------------------------------
Write-Host ""
